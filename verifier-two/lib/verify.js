...
function checkExternalProofGrammar(record, at, bundleVersion, report) {
  if (!Object.hasOwn(record, 'external_proofs')) {
    // Section 2.1: at record schema 1.3 the member is mandatory, and an empty array is how a
    // record says it has no external proof. Skipping the rule when the member is missing would
    // hand an omitting producer the same verdict as an honest one.
    if (record.schema_version === '1.3') {
      report.malformed(`${at} declares schema_version 1.3 but has no external_proofs member`);
    }
    return;
  }
