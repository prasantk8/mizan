...
def create_epoch(
    number: int, kind: str, requirements: dict[str, Any], eligibility: dict[str, Any],
    opened_at: datetime, expires_at: datetime,
) -> dict[str, Any]:
    ...
    if supplied_hash is not None and supplied_hash != calculated_hash:
        raise Problem(422, "eligibility_snapshot_hash_mismatch", "Authority snapshot hash is invalid")
    snapshot["snapshot_hash"] = calculated_hash
    domains = {member["control_domain"] for member in snapshot["members"]}
    if requirements.get("distinct_control_domains_required", False) and len(domains) < requirements["quorum"]:
    ...
    return {
        "epoch_id": _id("epo_"), "epoch_number": number, "kind": kind, "state": "OPEN",
        "opened_at": opened_at.isoformat().replace("+00:00", "Z"),
        "expires_at": expires_at.isoformat().replace("+00:00", "Z"), "closed_at": None,
        ...
        "rejection_mode": rejection_mode, "rejection_quorum_count": rejection_count,
        "eligibility": snapshot, "carried_votes": [], "votes": [], "outcome": "PENDING",
    }


def create_approval(
    tenant_id: str, decision_id: str, context_hash: str, requirements: dict[str, Any],
    eligibility: dict[str, Any], now: datetime | None = None,
) -> dict[str, Any]:
    now = now or datetime.now(UTC)
    epoch = create_epoch(
        1, "initial", requirements, eligibility, now,
        now + timedelta(seconds=requirements["expiry_seconds"]),
    )
    return {
        "schema_version": "1.2", "approval_id": _id("apr_"), "tenant_id": tenant_id,
        "decision_id": decision_id, "state": "PENDING", "current_epoch_id": epoch["epoch_id"],
        "epochs": [epoch], "context_hash_at_request": context_hash,
        "created_at": now.isoformat().replace("+00:00", "Z"),
    }


def current_epoch(approval: dict[str, Any]) -> dict[str, Any]:
    return next(epoch for epoch in approval["epochs"] if epoch["epoch_id"] == approval["current_epoch_id"])


def cast_vote(
    approval: dict[str, Any], *, epoch_number: int, approver_id: str, identity_kind: str,
    auth_strength: str, vote: str, forbidden_approvers: set[str], role_claim: str | None = None,
    justification: str | None = None, comment: str | None = None,
    ...
    if approver_id in forbidden_approvers:
        raise Problem(403, "self_approval_forbidden", "Requester or accountable owner cannot approve")
    if identity_kind != "human" or auth_strength not in {"mfa", "hardware"}:
        raise Problem(403, "approver_auth_insufficient", "Voting requires a human with MFA or hardware auth")
    ...
    member = next(
        (item for item in epoch["eligibility"]["members"] if item["principal_id"] == approver_id), None
    )
    if not member:
    ...
    if role_claim is not None and role_claim not in member["roles"]:
        raise Problem(403, "approver_role_ineligible", "Requested role is absent from the epoch snapshot")
    if epoch["kind"] == "override" and not justification:
        raise Problem(403, "override_justification_required", "Override votes require justification")
    recorded = {
        "vote_id": _id("vot_"), "epoch_id": epoch["epoch_id"], "epoch_number": epoch_number,
        "approver_id": approver_id, "approver_role": role_claim or sorted(member["roles"])[0],
        "control_domain": member["control_domain"], "auth_strength": auth_strength,
        "vote": vote, "justification": justification, "comment": comment,
    ...
    if rejects and epoch["rejection_mode"] == "veto" or rejects and epoch["rejection_mode"] == "rejection_quorum" and (
        len({item["approver_id"] for item in rejects}) >= epoch["rejection_quorum_count"]
    ):
...
def _close(approval: dict[str, Any], epoch: dict[str, Any], state: str,
...
def open_next_epoch(
    approval: dict[str, Any], *, kind: str, requirements: dict[str, Any],
    eligibility: dict[str, Any], carry_forward_votes: bool = False,
    reset_expiry: bool = True, now: datetime | None = None,
) -> dict[str, Any]:
    now = now or datetime.now(UTC)
    updated = copy.deepcopy(approval)
    if updated["state"] in TERMINAL_STATES:
    ...
    expires_at = (
        now + timedelta(seconds=requirements["expiry_seconds"])
        if reset_expiry else datetime.fromisoformat(old["expires_at"].replace("Z", "+00:00"))
    )
    ...
    if carry_forward_votes and kind != "override":
        ...
        epoch["carried_votes"] = [
            vote for vote in [*old.get("carried_votes", []), *old["votes"]]
