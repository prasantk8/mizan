...
class RegistryRepository(PostgresAuthorizationRepository):
        ...
        try:
            with self.pool.connection() as connection, connection.transaction():
                ...
                connection.execute(
                    ...
                    (tenant_id,document["agent_id"],document["version"],document["lifecycle_state"],
                     document.get("parent_agent_id"),json.dumps(document),document["created_at"],document["updated_at"]),
                )
            ...
            raise Problem(422, "registry_reference_missing", "Agent references an unknown object") from exc
        ...
        try:
            with self.pool.connection() as connection, connection.transaction():
                ...
                connection.execute(
                    ...
                    (tenant_id,profile["profile_id"],profile["profile_version"],profile["canonicalization"],
                     json.dumps(profile["bound_pointers"]),json.dumps(profile["volatile_pointers"]),profile_hash),
                )
                connection.execute(
                    ...
                    (tenant_id,document["tool_id"],profile["profile_id"],profile["profile_version"],json.dumps(document)),
                )
            ...
            raise Problem(409, "tool_exists", "tool or binding-profile version already exists") from exc
    ...
    def create_policy(self, tenant_id: str, document: dict[str, Any]) -> dict[str, Any]:
        ...
        if document["content_hash"] != expected_hash:
            raise Problem(400, "content_hash_mismatch", "Policy content_hash is not canonical source hash")
        try:
            with self.pool.connection() as connection, connection.transaction():
                self._scope(connection, tenant_id)
                connection.execute(
                    ...
                    (tenant_id,document["policy_id"],document["version"],document["status"],
                     document.get("effective_from"),document["decision"],document["content_hash"],
    ...
    def get(self, tenant_id: str, kind: ResourceKind, identifier: str, version: int | None = None) -> dict:
        table, id_column = self._RESOURCE_MAP[kind]
        version_clause = " AND version=%s" if kind == "policies" and version is not None else ""
        params: tuple[Any, ...] = (tenant_id, identifier, version) if version_clause else (tenant_id, identifier)
    ...
    def list(self, tenant_id: str, kind: ResourceKind, limit: int, cursor: str | None) -> Page:
        ...
        next_cursor = encode_cursor(rows[limit - 1][1], rows[limit - 1][2]) if len(rows) > limit else None
