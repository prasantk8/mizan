...
class ExecutionTokenCodec:
    def __init__(self, issuer: str, private_key: Ed25519PrivateKey,
                 public_key: Ed25519PublicKey | None = None, clock_skew_seconds: int = 30) -> None:
        self.issuer = issuer
    ...
    def decode(self, token: str) -> dict[str, Any]:
        try:
            return jwt.decode(
                token,self.public_key,algorithms=["EdDSA"],issuer=self.issuer,
                audience="mizan-execution-gateway",leeway=self.clock_skew_seconds,
                options={"require":["exp","iat","nbf","iss","aud","jti"]},
            )
        except jwt.PyJWTError as exc:
            raise Problem(403, "execution_token_invalid", "Execution capability is invalid or expired") from exc


class ExecutionService:
    def __init__(self, database_url: str, codec: ExecutionTokenCodec) -> None:
        ...
        with self.pool.connection() as connection, connection.transaction():
            ...
            if adr["decision"] == "REQUIRE_APPROVAL":
                ...
                if not approval or approval[0]["state"] not in {"APPROVED","OVERRIDDEN"}:
                    raise Problem(403, "approval_incomplete", "Approval has not reached an executable state")
            ...
            jti = uuid4().hex
            claims = {
                "token_version":"1.2","jti":jti,"iss":self.codec.issuer,
                "aud":"mizan-execution-gateway","tenant_id":tenant_id,
                "agent_id":adr["agent"]["id"],"principal_id":adr["principal"]["id"],
                "delegation_chain_hash":canonical_hash(adr["agent"]["delegation_chain"]),
                "authorized_executor":executor,"decision_id":decision_id,"tool_id":adr["tool"]["id"],
                "parameters_hash":adr["tool"]["parameters_hash"],
                "binding_profile":adr["tool"]["binding_profile"],"context_hash":adr["context_hash"],
                "approval_epoch_id":approval_epoch,
                "constraints_hash":canonical_hash(adr.get("constraints")) if adr.get("constraints") else None,
                "iat":int(now.timestamp()),"nbf":int(now.timestamp()),"exp":int((now+timedelta(seconds=ttl)).timestamp()),
            }
            connection.execute(
                ...
                (tenant_id,_jti_hash(jti),decision_id,claims["agent_id"],claims["tool_id"],executor,
                 json.dumps(claims),datetime.fromtimestamp(claims["exp"],UTC)),
            )
            append_decision_event_tx(
                connection,tenant_id,decision_id,"CAPABILITY_ISSUED",
                {"kind":"system","id":"mizan-execution-service","authenticated_workload":None},
                {"token_jti_hash":_jti_hash(jti),"approval_state":None},now,
            )
            return self.codec.encode(claims)

    def redeem(
        self, token: str, decision_id: str, peer_spiffe: str,
        ...
        with self.pool.connection() as connection, connection.transaction():
            ...
            if idempotency_key:
                ...
                if existing:
                    if existing[0]["authorized_executor"] != peer_spiffe:
                        raise Problem(403, "lease_executor_mismatch", "Existing lease belongs to another executor")
                    return existing[0]
            row = connection.execute(
            ...
            if not row or row[0] is not None:
                raise Problem(403, "execution_token_consumed", "Execution capability is absent or consumed")
            ...
            lease = {
                "schema_version":"1.2","lease_id":lease_id,"redeemed_jti":claims["jti"],
                "tenant_id":tenant_id,"agent_id":claims["agent_id"],"principal_id":claims["principal_id"],
                "authorized_executor":peer_spiffe,"decision_id":decision_id,"tool_id":claims["tool_id"],
                "state":"LEASED","idempotency_key":idempotency_key,
                "granted_at":now.isoformat().replace("+00:00","Z"),
                "expires_at":expires.isoformat().replace("+00:00","Z"),"last_heartbeat_at":None,
                "heartbeat_interval_seconds":execution["heartbeat_interval_seconds"],
                "extensions_used":0,"max_extensions":execution["max_lease_extensions"],"result_hash":None,
            }
            try:
                connection.execute(
                    ...
                    (tenant_id,lease_id,_jti_hash(claims["jti"]),decision_id,claims["agent_id"],
                     claims["tool_id"],claims["principal_id"],peer_spiffe,"LEASED",idempotency_key,0,
                     execution["max_lease_extensions"],execution["heartbeat_interval_seconds"],
                     json.dumps(lease),expires),
                )
            except UniqueViolation as exc:
                raise Problem(409, "execution_idempotency_conflict", "Idempotency key is already used") from exc
            ...
            if updated != 1:
                raise Problem(403, "execution_token_consumed", "Execution capability lost redemption race")
            append_decision_event_tx(
                connection,tenant_id,decision_id,"LEASE_STARTED",
                {"kind":"service","id":peer_spiffe,"authenticated_workload":peer_spiffe},
                {"lease_id":lease_id},now,
            )
            return lease

    def heartbeat(self, tenant_id: str, decision_id: str, lease_id: str,
                  peer_spiffe: str) -> dict[str, Any]:
        return self._transition_lease(tenant_id,decision_id,lease_id,peer_spiffe,"heartbeat",None)

    def complete(self, tenant_id: str, decision_id: str, lease_id: str,
                 peer_spiffe: str, result_hash: str | None, failure_code: str | None) -> dict[str, Any]:
        return self._transition_lease(
            tenant_id,decision_id,lease_id,peer_spiffe,"complete",
            {"result_hash":result_hash,"failure_code":failure_code},
        )

    def _transition_lease(self, tenant_id: str, decision_id: str, lease_id: str,
                          peer_spiffe: str, operation: str,
        ...
        with self.pool.connection() as connection, connection.transaction():
            ...
            row = connection.execute(
                ...
                "AND decision_id=%s FOR UPDATE", (tenant_id,lease_id,decision_id),
            ).fetchone()
            if not row:
            ...
            if now >= datetime.fromisoformat(lease["expires_at"].replace("Z","+00:00")):
                ...
                append_decision_event_tx(
                    connection,tenant_id,decision_id,"LEASE_EXPIRED",
                    {"kind":"system","id":"mizan-execution-service","authenticated_workload":None},
                    {"lease_id":lease_id},now,
                )
            ...
            if operation == "heartbeat":
                if lease["extensions_used"] >= lease["max_extensions"]:
                    raise Problem(409, "lease_extension_exhausted", "Lease extension budget is exhausted")
                ...
                event_type = "EXECUTION_FAILED" if outcome["failure_code"] else "EXECUTION_COMPLETED"
                payload = {"lease_id":lease_id,**outcome}
            self._save_lease(connection, tenant_id, lease)
            append_decision_event_tx(
                connection,tenant_id,decision_id,event_type,
                {"kind":"service","id":peer_spiffe,"authenticated_workload":peer_spiffe},payload,now,
            )
            return lease
    ...
    def _save_lease(connection: Any, tenant_id: str, lease: dict[str, Any]) -> None:
        connection.execute(
            ...
            (lease["state"],lease["extensions_used"],json.dumps(lease),lease["expires_at"],
             tenant_id,lease["lease_id"]),
        )

    @staticmethod
    def _revalidate(connection: Any, claims: dict[str, Any], adr: dict[str, Any],
                    tool: dict[str, Any], agent_state: str) -> None:
        expected = {
            "agent_id":adr["agent"]["id"],"principal_id":adr["principal"]["id"],
            "tool_id":adr["tool"]["id"],"parameters_hash":adr["tool"]["parameters_hash"],
            "context_hash":adr["context_hash"],"binding_profile":adr["tool"]["binding_profile"],
        }
        if any(claims[key] != value for key, value in expected.items()):
            raise Problem(403, "execution_context_drift", "Capability no longer matches decision context")
    ...
    @staticmethod
    def _require_receipts(connection: Any, tenant_id: str, decision_id: str,
                          adr: dict[str, Any]) -> None:
        ...
        if not adr_receipt:
            raise Problem(403, "immutable_receipt_missing", "Financial decision is not durably published")
        if adr["approval"]["required"]:
            approval_receipt = connection.execute(
                """SELECT 1 FROM mizan.decision_events de JOIN mizan.evidence_receipts er
                    ...
                    ORDER BY de.decision_sequence DESC LIMIT 1""", (tenant_id,decision_id),
            ).fetchone()
            if not approval_receipt:
                raise Problem(403, "approval_receipt_missing", "Approval evidence is not durably published")
