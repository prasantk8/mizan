...
class ApprovalRepository:
    ...
    def authority_snapshot(self, connection: Any, tenant_id: str, roles: list[str]) -> dict:
        ...
        if not row:
            raise Problem(422, "authority_mapping_missing", "No approved role-authority mapping exists")
        members = [
            member for member in row[1]["members"] if set(member["roles"]) & set(roles)
        ]
        if not members:
        ...
        return {
            ...
            "authority_source": "mizan_role_registry", "authority_mapping_version": row[0],
            "roles": roles, "members": members,
        }

    def create(
        self, tenant_id: str, decision_id: str, requester_id: str,
        forbidden_approvers: set[str], context_hash: str, controls: dict[str, Any],
    ) -> dict[str, Any]:
        with self.pool.connection() as connection, connection.transaction():
            ...
            connection.execute(
                ...
                (tenant_id,approval["approval_id"],decision_id,approval["state"],epoch["epoch_id"],
                 requester_id,json.dumps(controls),json.dumps(sorted(forbidden_approvers)),
                 json.dumps(approval),approval["created_at"]),
            )
            self._insert_epoch(connection, tenant_id, approval["approval_id"], epoch)
            append_decision_event_tx(
                connection,tenant_id,decision_id,"APPROVAL_EPOCH_OPENED",SYSTEM_ACTOR,
                {"approval_id":approval["approval_id"],"epoch_id":epoch["epoch_id"],
                 "approval_state":"PENDING"},datetime.now(UTC),
            )
            return approval
    ...
    def vote(
        self, tenant_id: str, approval_id: str, principal: AuthenticatedPrincipal,
        ...
        with self.pool.connection() as connection, connection.transaction():
            ...
            updated, recorded = cast_vote(
                row["document"], epoch_number=request["epoch_number"],
                approver_id=principal.principal_id, identity_kind=principal.identity_kind,
                auth_strength=principal.auth_strength, vote=request["vote"],
                forbidden_approvers=set(row["forbidden_approvers"]),
                role_claim=request.get("role_claim"), justification=request.get("justification"),
            ...
            connection.execute(
                ...
                (tenant_id,recorded["vote_id"],approval_id,recorded["epoch_id"],
                 recorded["approver_id"],recorded["control_domain"],recorded["vote"],
                 recorded["timestamp"],json.dumps(recorded)),
            )
            self._update(connection, tenant_id, updated)
            append_decision_event_tx(
                connection,tenant_id,updated["decision_id"],"APPROVAL_VOTE_CAST",
                {"kind":"human","id":principal.principal_id,"authenticated_workload":None},
                {"approval_id":approval_id,"epoch_id":recorded["epoch_id"],
                 "vote_id":recorded["vote_id"],"approval_state":updated["state"]},datetime.now(UTC),
            )
            if updated["state"] in {"APPROVED","REJECTED","OVERRIDDEN","REVIEW_REQUIRED"}:
                append_decision_event_tx(
                    connection,tenant_id,updated["decision_id"],"APPROVAL_RESOLVED",SYSTEM_ACTOR,
                    {"approval_id":approval_id,"epoch_id":epoch["epoch_id"],
                     "approval_state":updated["state"]},datetime.now(UTC),
                )
            return updated
    ...
    def escalate(self, tenant_id: str, approval_id: str) -> dict[str, Any]:
        with self.pool.connection() as connection, connection.transaction():
            ...
            updated = open_next_epoch(
                row["document"],kind="escalation",requirements=requirements,eligibility=snapshot,
                carry_forward_votes=config["carry_forward_votes"],reset_expiry=config["reset_expiry"],
            )
    ...
    def override(
        ...
        with self.pool.connection() as connection, connection.transaction():
            ...
            if not config or not set(principal.roles) & set(config["eligible_roles"]):
                raise Problem(403, "override_forbidden", "Override is absent or caller is ineligible")
            snapshot = self.authority_snapshot(connection, tenant_id, config["eligible_roles"])
            requirements = {
                "quorum":config["quorum"],"expiry_seconds":row["controls"]["expiry_seconds"],
                "rejection_mode":"veto",
                "distinct_control_domains_required":config.get("distinct_control_domains_required",True),
            }
            updated = open_next_epoch(
                row["document"],kind="override",requirements=requirements,eligibility=snapshot,
                carry_forward_votes=False,reset_expiry=True,
            )
            self._update_with_new_epoch(connection, tenant_id, updated)
            self._event_opened(connection, tenant_id, updated)
            connection.execute(
                ...
                (tenant_id,approval_id,json.dumps({"severity":"high","notify":config.get("notify",[])})),
            )
            return updated
    ...
    def withdraw(self, tenant_id: str, approval_id: str, requester_id: str) -> dict[str, Any]:
        with self.pool.connection() as connection, connection.transaction():
            ...
            append_decision_event_tx(
                connection,tenant_id,updated["decision_id"],"APPROVAL_RESOLVED",SYSTEM_ACTOR,
                {"approval_id":approval_id,"epoch_id":updated["current_epoch_id"],
                 "approval_state":"WITHDRAWN"},datetime.now(UTC),
            )
            return updated
    ...
    def _locked(connection: Any, tenant_id: str, approval_id: str) -> dict[str, Any]:
        row = connection.execute(
            ...
            "WHERE tenant_id=%s AND approval_id=%s FOR UPDATE", (tenant_id, approval_id),
        ).fetchone()
        if not row:
            raise Problem(404, "approval_not_found", "Approval was not found")
        return {"document":row[0],"controls":row[1],"forbidden_approvers":row[2],"requester_id":row[3]}

    @staticmethod
    def _insert_epoch(connection: Any, tenant_id: str, approval_id: str, epoch: dict[str, Any]) -> None:
        connection.execute(
            ...
            (tenant_id,epoch["epoch_id"],approval_id,epoch["epoch_number"],epoch["state"],
             json.dumps(epoch["eligibility"]),json.dumps(epoch),epoch["opened_at"],epoch["expires_at"],
             epoch.get("closed_at")),
        )
    ...
    def _update(connection: Any, tenant_id: str, approval: dict[str, Any]) -> None:
        ...
        connection.execute(
            ...
            (approval["state"],epoch["epoch_id"],json.dumps(approval),tenant_id,approval["approval_id"]),
        )
        connection.execute(
            "UPDATE mizan.approval_epochs SET state=%s,document=%s,closed_at=%s "
            "WHERE tenant_id=%s AND epoch_id=%s",
            (epoch["state"],json.dumps(epoch),epoch.get("closed_at"),tenant_id,epoch["epoch_id"]),
        )

    def _update_with_new_epoch(self, connection: Any, tenant_id: str, approval: dict[str, Any]) -> None:
        previous, epoch = approval["epochs"][-2:]
        connection.execute(
            "UPDATE mizan.approval_epochs SET state=%s,document=%s,closed_at=%s "
            "WHERE tenant_id=%s AND epoch_id=%s",
            (previous["state"],json.dumps(previous),previous["closed_at"],tenant_id,previous["epoch_id"]),
        )
        self._insert_epoch(connection, tenant_id, approval["approval_id"], epoch)
    ...
    def _epoch_requirements(controls: dict[str, Any], old: dict[str, Any]) -> dict[str, Any]:
        return {
            "quorum":old["quorum"],"expiry_seconds":controls["expiry_seconds"],
    ...
    def _event_opened(connection: Any, tenant_id: str, approval: dict[str, Any]) -> None:
        epoch = current_epoch(approval)
        append_decision_event_tx(
            connection,tenant_id,approval["decision_id"],"APPROVAL_EPOCH_OPENED",SYSTEM_ACTOR,
            {"approval_id":approval["approval_id"],"epoch_id":epoch["epoch_id"],
             "approval_state":approval["state"]},datetime.now(UTC),
        )
