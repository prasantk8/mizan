...
| Claim | Backed by | Status |
|---|---|---|
| Audit commitment key custody | `keys.py` (`MacKey`, `KeyProvider.active_mac_key`, `commitment_keyset`), `vault_transit.py::mac`, `/v1/audit/commitment-keys`, `scripts/provision_vault.sh` | **shipped** — B-30 ruled 2026-09-02, delivered by T-054. ADR-004 G.1 is amended from four key roles to **four signing plus one MAC**; `MIZAN_AUDIT_HMAC_KEY_REF` finally has custody, closing TM-001 **R-2**. Exercised against a real Vault in CI job `vault-transit`, including the refusal to HMAC under an `ed25519` key. The commitment keyset is served, **never** exported: an HMAC has no public half, and both verifiers plus the exporter would reject an entry carrying none |
| Redaction / DLP attestation | `security/mizan_security/redaction.py`, and `EvidenceRepository.append_audit` (`control-plane/mizan_control_plane/evidence.py:453`) | **still unwired at both ends, but no longer blocked** — the module, the repository method that enforces its attestation, and their tests all exist; neither has a production caller (`append_audit`'s only caller in the tree is `tests/integration/test_authorize_postgres.py`). The wall was custody, and T-054 removed it: `Redactor` now takes a `MacKey` rather than raw key bytes, so no private key material enters the process under `custody=kms`. **T-146** is now the wiring task it was originally advertised as. Deleting was never the cheap alternative: invariants **I-12/I-18/I-19** and SPEC §2.5's required `AuditTrail` members would be left unimplemented, which is a §0 change |
