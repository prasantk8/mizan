...
Bundle 1.1 adds one record member, `external_proofs`. Presence is decided by the record's own
`schema_version`, not by whether the record has anything to prove: a record declaring
`schema_version` `"1.3"` MUST carry `external_proofs`, and MUST carry it as an empty array when no
external proof applies; a record declaring any earlier `schema_version` MUST NOT carry it. A
verifier MUST report `MALFORMED` for a record that declares `schema_version` `"1.3"` and has no
`external_proofs` member. An empty array and an absent member are therefore different statements:
the first says this record has no external proof, the second says the record predates the member,
and a producer that silently dropped the array would otherwise be indistinguishable from one that
had nothing to report.

Bundle 1.0 records MUST NOT carry the member at all; this makes a new proof-bearing bundle fail
loudly in a verifier that only understands 1.0 rather than silently ignoring the new evidence. A
producer therefore declares `bundle_version="1.1"` exactly when some record in the bundle carries
`external_proofs`, and `"1.0"` otherwise, so a history written before the member existed stays
verifiable by a 1.0-only verifier.
