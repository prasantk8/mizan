...
> **Status 2026-09-02 — delivered as one PR (#41), Mizan side.** T-133–T-138 merged together rather
> than one PR per task, on the founder's instruction to expedite the workstream. Two things a reader
> of this table should not have to discover for themselves:
>
> * **Two rows of the UC-2 decision matrix were wrong about the system, and the catalogue was
>   corrected rather than the tests.** An *expired* proof is refused inside the verifier — HTTP 403,
>   no ADR written — not the evidenced DENY this plan assumed. And no shipped read policy exists, so
>   "read client profile → ALLOW" is a deployment's own composition, not something
>   `policies/reference/` delivers.
> * **Three limits are stated, not closed:** the `jti` replay set is per process (so replay refusal is
>   not deployment-wide), the Memtara keyset is never refreshed (so a rotation is an outage until
>   restart), and the chain head is recorded but unauthenticated until **M-04**. All three are in
>   `MODULE_LEDGER.md`, `ADR-010` and the claims register.
>
> The critical path now runs through **M-01 and M-04** on the Memtara side. T-135 is done in the sense
> that both verifiers check a proof against an operator-supplied keyset; it is *not* done in the sense
> of "one record, both chain heads" until M-04 makes the Memtara half verifiable.
