...
| **UC-2** | **Advised-sales suitability: proof-gated recommendation** | **Mizan + Memtara** | **Head of Wealth + CRO / Compliance** | **Technical Preview** | Yes (`scripts/demo_memtara_walk.py`) | T-133–T-138, M-01–M-06 |
...
Recommend product with expired / wrong-ISIN proof→ DENY
...
| Mizan verifying the token and gating the tool on it | Technical Preview — T-133/T-134 |
| One cross-anchored bundle | Technical Preview — T-135; Memtara-side DecisionEvidence enrichment remains M-04 |
...
| Head of Wealth | UC-2 | "The bank never holds the figures; the recommendation cannot run without the proof." | Case File sample; UC-2 two-console recording |
