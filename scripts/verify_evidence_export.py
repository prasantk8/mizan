...
def validate_external_proof_grammar(record: dict[str, Any], sequence: int, version: str) -> None:
    if "external_proofs" not in record:
        # Section 2.1 makes the member required at ADR_Record schema 1.3, empty array included.
        # Returning early on absence would have made "required" mean nothing: a producer that
        # dropped the array would read as a record with no proof to report, and the two are
        # exactly what the empty array exists to tell apart.
        if record.get("schema_version") == "1.3":
            raise MalformedBundle(
                f"record {sequence} declares schema_version 1.3 and omits external_proofs, "
                "which that schema requires even when no external proof applies"
            )
        return
    if version != "1.1":
        raise MalformedBundle(
...
def main() -> int:
    ...
    # A trust root is something the operator typed, not something the bundle contains. A path
    # that is not there, or cannot be opened, is a mistake on the command line, and answering
    # MALFORMED would blame the evidence for it -- the one accusation this verifier exists to
    # make carefully. Checked here rather than inside the bundle walk so it is reported even when
    # the bundle carries no proof that would have caused the file to be read at all.
    for path in args.memtara_trust_root:
        try:
            with path.open("rb"):
                pass
        except OSError as exc:
            parser.error(f"--memtara-trust-root {path} cannot be read: {exc.strerror or exc}")
