...
def identity(tenant_id: str = TENANT) -> AuthenticatedIdentity:
    return AuthenticatedIdentity(
        tenant_id=tenant_id, agent_id=AGENT, subject="spiffe://mizan/agent/wealth-01",
...
def service() -> tuple[AuthorizationService, InMemoryAuthorizationRepository]:
    repository = InMemoryAuthorizationRepository(
        agents=[RegistryAgent(
            tenant_id=TENANT, agent_id=AGENT, version="1.0.0",
            lifecycle_state="ACTIVE", permitted_tools={TOOL},
        )],
        tools=[RegistryTool(
            tenant_id=TENANT, tool_id=TOOL, risk_tier="HIGH", resource_owner="core-banking",
            data_classification="financial", profile_id="bp_transfer-v1", profile_version=1,
            bound_pointers=["/amount"], volatile_pointers=["/request_time"],
            executor_spiffe_ids=["spiffe://mizan/executor/wealth"],
        )],
    )
    return AuthorizationService(repository, RegistryFloorRiskProvider(), "test", "f" * 64), repository
