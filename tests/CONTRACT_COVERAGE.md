...
| ADR-004 G.19 (publication at rest, lag SLO, poison quarantine) | `unit/test_outbox_worker.py::test_one_unpublishable_stream_does_not_block_the_others` + `unit/test_outbox_worker.py::test_quarantined_rows_are_excluded_from_the_lag_but_open_their_own_breaker` + `unit/test_outbox_worker.py::test_publication_lag_beyond_the_slo_opens_the_breaker_and_recovery_closes_it` + `integration/test_outbox_worker_postgres.py::test_one_worker_tick_publishes_evidence_and_relays_events_that_have_no_receipt` |
| ADR-007 Amd. (expiry is reached, not reported) + §4 `mizan.approval.expired` | `integration/test_outbox_worker_postgres.py::test_an_expired_approval_epoch_is_reached_at_rest_with_its_section_4_event` + `integration/test_outbox_worker_postgres.py::test_the_sweeper_leaves_an_approval_that_still_has_time_alone` + `unit/test_outbox_worker.py::test_expire_refuses_an_approval_that_already_resolved` |
| ADR-008 Amd. (a lapsed lease lapses unasked) + §4 `mizan.execution.lease_expired` | `integration/test_outbox_worker_postgres.py::test_a_lapsed_lease_becomes_lease_expired_at_rest_with_its_section_4_event` |
| ADR-004 G.20 (§2 `trace_id` is a W3C trace-id, `span_id` is populated) | `integration/test_observability_postgres.py::test_the_trace_a_caller_sent_is_the_trace_the_signed_record_names` + `unit/test_trace_contract.py::test_the_trace_id_recorded_is_the_callers_trace_and_not_a_hash_of_the_request_id` + `unit/test_trace_contract.py::test_the_recorded_span_is_the_span_that_decided_rather_than_null` + `unit/test_trace_contract.py::test_a_decision_made_with_no_ambient_trace_still_names_a_real_one` |
| ADR-004 G.20 (propagation unconditional, export refused when misconfigured) | `unit/test_observability.py::test_no_collector_configured_is_a_working_tracer_not_a_disabled_one` + `unit/test_observability.py::test_a_configured_collector_with_no_exporter_installed_refuses_to_start` + `unit/test_observability.py::test_an_exported_span_carries_the_same_ids_the_evidence_will_name` |
| ADR-003 Amd. D (an audit sink may not decide the answer it audits) | `unit/test_execution.py::test_a_security_event_sink_that_fails_does_not_change_the_security_answer` + `unit/test_execution.py::test_a_dropped_security_event_is_written_where_the_siem_can_still_recover_it` |
| ADR-003 Amd. D (breaker state is a sample, not an edge) | `unit/test_outbox_worker.py::test_the_breaker_gauge_keeps_asserting_while_it_is_open` + `unit/test_outbox_worker.py::test_a_log_line_that_names_its_own_tenant_does_not_crash_the_worker` |
| ADR-005 (ambient tenant context cannot outlive its scope) | `unit/test_observability.py::test_a_tenant_can_never_be_annotated_onto_the_next_unit_of_work` + `unit/test_observability.py::test_a_secret_never_reaches_a_log_line_however_it_is_passed` |
| SPEC §7 (drain-lag SLO and latency targets are measurable, not narrated) | `unit/test_observability.py::test_every_section_7_latency_target_is_a_bucket_edge` + `unit/test_outbox_worker.py::test_the_lag_gauge_is_read_from_the_database_not_accumulated_in_the_process` + `unit/test_outbox_worker.py::test_a_worker_that_stopped_ticking_is_distinguishable_from_a_quiet_tenant` |
| SPEC §8 `MIZAN_METRICS_PORT` / `MIZAN_LOG_FORMAT` (private listener, bounded cardinality) | `integration/test_observability_postgres.py::test_the_trace_a_caller_sent_is_the_trace_the_signed_record_names` + `unit/test_observability.py::test_the_metrics_listener_serves_the_exposition_and_nothing_else` + `unit/test_observability.py::test_an_identifier_can_never_become_a_metric_label` + `unit/test_observability.py::test_latency_is_recorded_against_the_route_template_not_the_request_path` |
